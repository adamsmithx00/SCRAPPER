...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/2127221861/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/2127221861')
   open(f"Users/2127221861/phone.csv","w")
if not os.path.exists('data.csv'):
    open("data.csv","w")
APP_ID = 11573285
API_HASH = "f2cc3fdc32197c8fbaae9d0bf69d2033"
BOT_TOKEN = " <bot token> "
UPDATES_CHANNEL = "AbOuT_xNKIT"
OWNER= [2127221861]
PREMIUM=[2127221861]
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Sorry Sir, You are Banned. Contact My [Support Group](https://t.me/AbOuT_xNKIT).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
         ...
         await app.send_message(chat_id=message.chat.id, text="**Something Went Wrong. Contact My [Support Group](https://t.me/AbOuT_xNKIT).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
...
async def phone(lel, message):
 try:
   ...
   if message.from_user.id not in PREMIUM:
      await app.send_message(message.chat.id, f"**Made with ❤️ By @AbOuT_xNKIT**")
      return
   if not os.path.exists(f"Users/{message.from_user.id}/phone.csv"):
      os.mkdir(f'./Users/{message.from_user.id}')
      open(f"Users/{message.from_user.id}/phone.csv","w")
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/2127221861/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/2127221861/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
...
if not os.path.exists(f"Users/5122474448/phone.csv"):

   os.mkdir('./Users')

   os.mkdir(f'./Users/5122474448')

   open(f"Users/5122474448/phone.csv","w")
...
BOT_TOKEN = "5175623354:AAHnA2_5emOg0oTcwzXtOiVKyfzKjXJhSHU"
